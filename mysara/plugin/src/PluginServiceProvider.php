...
use MySara\Panel\Middleware\SetPanelLocale;
use MySara\Plugin\Core\PluginManager;
    ...
    private function registerBladeInsertDirectives(): void
    {
        Blade::directive('hookinsert', function ($parameter) {
            ...
            return ' <?php
                ...
                '.$definedVars.'
                $output = \MySara\Plugin\Core\Blade\Hook::getSingleton()->getHook("'.$name.'",["data"=>$__definedVars],function($data) { return null; });
                if ($output)
                echo $output;
                ?>';
        });
    ...
    private function registerBladeUpdateDirectives(): void
    {
        ...
        Blade::directive('endhookupdate', function () {
            return ' <?php
                ...
                $output = \MySara\Plugin\Core\Blade\Hook::getSingleton()->getWrapper("$__hook_name",["data"=>$__definedVars],function($data) { return null; },$__hook_content);
                unset($__hook_name);
