...
use MySara\Common\Models\CartItem;
use MySara\Common\Models\Order;
use MySara\Common\Repositories\CartItemRepo;
use MySara\Common\Resources\CartListItem;
use Throwable;
...
class CartService
{
    ...
    public function addCart($data): array
    {
        ...
        if (! empty($data['options'])) {
            $productSku = \MySara\Common\Models\Product\Sku::find($data['sku_id']);
            if ($productSku && $productSku->product) {
                $optionService = \MySara\Common\Services\ProductOptionService::getInstance($productSku->product);
    ...
    {
        // 先删除已有的选项值
        \MySara\Common\Models\Cart\OptionValue::where('cart_item_id', $cartItem->id)->delete();
        ...
        foreach ($options as $optionId => $optionValueIds) {
            ...
            foreach ($optionValueIds as $optionValueId) {
                // 获取选项和选项值信息
                $option      = \MySara\Common\Models\Option::find($optionId);
                $optionValue = \MySara\Common\Models\OptionValue::find($optionValueId);
                ...
                // 获取产品选项值配置中的价格调整
                $productOptionValue = \MySara\Common\Models\Product\OptionValue::where([
                ...
                // 创建购物车选项值记录
                \MySara\Common\Models\Cart\OptionValue::create([
