...
use MySara\Common\Libraries\ApiHook;
use MySara\Common\Libraries\Currency;
use MySara\Common\Libraries\ViewHook;
use MySara\Common\Libraries\Weight;
use MySara\Common\Repositories\CurrencyRepo;
use MySara\Common\Repositories\LocaleRepo;
use MySara\Common\Repositories\SettingRepo;
use MySara\Common\Services\ImageService;
use MySara\Common\Support\Registry;
...
if (! function_exists('mysara_version')) {
    /**
     * Generate an asset path for the application.
     *
     * @return string
     */
    function mysara_version(): string
    {
        $default = ucfirst(config('mysara.edition')).' v'.config('mysara.version').'('.config('mysara.build').')';

        return fire_hook_filter('mysara.version.display', $default);
    }
}

if (! function_exists('mysara_brand_link')) {
    /**
     * Get mysara brand link
     *
     * @return string
     */
    function mysara_brand_link(): string
    {
        if (is_admin()) {
            $default = '<a href="https://www.mysara.com" class="ms-2" target="_blank">MySara</a>';
        } else {
            $default = 'Powered By <a href="https://www.mysara.com" class="ms-2" target="_blank">MySara</a>';
        }

        return fire_hook_filter('mysara.brand.link.display', $default);
    }
}
...
if (! function_exists('seller_enabled')) {
    ...
    {
        return class_exists('\MySara\Seller\SellerServiceProvider') && env('SELLER_ENABLED', true);
    }
}
